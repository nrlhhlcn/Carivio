...
import { Mail, Lock, Eye, EyeOff, ArrowRight, Flag, Check, Settings, Menu, Lock as LockIcon, FileText, TrendingUp, Briefcase, Star, Target, Award, Server, Cpu, Monitor, Database, Cloud, BarChart3, PieChart, Activity, Home } from "lucide-react"
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-white transition-all duration-700 ease-in-out animate-slide-in-left">
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="text-center animate-fade-in-up" style={{animationDelay: '0.2s'}}>
            ...
            <p className="text-gray-500">CV analiz platformunuza hoş geldiniz</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 animate-fade-in-up" style={{animationDelay: '0.4s'}}>
            ...
            <div className="flex items-center justify-between">
              ...
              <Link href="/forgot-password" className="text-sm" style={{color: '#0065F8'}} onMouseEnter={(e) => e.target.style.color = '#4300FF'} onMouseLeave={(e) => e.target.style.color = '#0065F8'}>
                Şifremi Unuttum?
              </Link>
            </div>
            ...
            <Button
              ...
              onMouseEnter={(e) => e.target.style.background = 'linear-gradient(135deg, #0065F8 0%, #4300FF 100%)'}
              onMouseLeave={(e) => e.target.style.background = 'linear-gradient(135deg, #4300FF 0%, #0065F8 100%)'}
              disabled={isLoading}
            >
          ...
          <div className="text-center text-sm text-gray-600 animate-fade-in-up" style={{animationDelay: '0.8s'}}>
            Hesabınız yok mu?{" "}
            <Link href="/register" className="font-medium" style={{color: '#0065F8'}} onMouseEnter={(e) => e.target.style.color = '#4300FF'} onMouseLeave={(e) => e.target.style.color = '#0065F8'}>
              Kayıt Ol
            </Link>
          </div>
      ...
      <div className="flex-1 relative flex items-center justify-center p-8 transition-all duration-700 ease-in-out animate-slide-in-right" style={{background: 'linear-gradient(135deg, #4300FF 0%, #0065F8 50%, #4300FF 100%)'}}>
        ...
        <div className="relative z-10 flex items-center justify-center h-full">
          <div className="relative w-full h-full flex items-center justify-center">
            {/* CV Analysis Banner Image with Animations */}
            <div className="relative w-full max-w-md h-auto group">
              {/* Floating background elements */}
              <div className="absolute -top-4 -left-4 w-8 h-8 bg-white/20 rounded-full animate-bounce" style={{animationDelay: '0.5s', animationDuration: '3s'}}></div>
              <div className="absolute -top-2 -right-6 w-6 h-6 bg-white/15 rounded-full animate-bounce" style={{animationDelay: '1s', animationDuration: '2.5s'}}></div>
              <div className="absolute -bottom-4 -left-2 w-4 h-4 bg-white/25 rounded-full animate-bounce" style={{animationDelay: '1.5s', animationDuration: '2s'}}></div>
              <div className="absolute -bottom-2 -right-4 w-5 h-5 bg-white/20 rounded-full animate-bounce" style={{animationDelay: '2s', animationDuration: '2.8s'}}></div>
              ...
              {/* Rotating elements around the image */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-28 h-28 pointer-events-none">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3 h-3 bg-yellow-400 rounded-full animate-spin" style={{animationDuration: '8s'}}></div>
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2 h-2 bg-blue-400 rounded-full animate-spin" style={{animationDuration: '6s', animationDirection: 'reverse'}}></div>
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-2.5 h-2.5 bg-green-400 rounded-full animate-spin" style={{animationDuration: '10s'}}></div>
                <div className="absolute right-0 top-1/2 -translate-y-1/2 w-2 h-2 bg-purple-400 rounded-full animate-spin" style={{animationDuration: '7s', animationDirection: 'reverse'}}></div>
              </div>
              
              {/* Pulse rings */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-36 h-36 border-2 border-white/20 rounded-full animate-ping" style={{animationDuration: '3s'}}></div>
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-44 h-44 border border-white/10 rounded-full animate-ping" style={{animationDuration: '4s', animationDelay: '1s'}}></div>
            </div>
          </div>
        </div>
      </div>
